...
function readAllOptionFiles() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!optionFiles.length) {
            throw new Error("No option files matching pattern 'opDDMMYYYY.csv' found in ~/Desktop/NSE-Data/data/option");
        }
        let allData = [];
        for (const file of optionFiles) {
            try {
                const fileData = yield parseOptionFile(path.join(OPTION_ROOT, file));
                allData = allData.concat(fileData);
            }
            catch (err) {
                console.warn(`Warning: Failed to parse file ${file}: ${err.message}`);
...
function readOptionFilesByDates(dates) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const d of dateArray) {
            if (!/^\d{8}$/.test(d)) {
                throw new Error(`Invalid date format '${d}'. Expected DDMMYYYY (e.g. 01072025)`);
            }
        }
        yield fsPromises.access(OPTION_ROOT);
        ...
        for (const fileName of filesToRead) {
            ...
            try {
                ...
                console.warn(`Warning: Failed to parse option file ${fileName}: ${err.message}`);
